...
module SCCB_controller (
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (p_counter <= 625 - 1) begin  // 500 -> 625
                tick  <= 0;
                o_clk <= 1'b0;
                if (a_reg) begin
                    p_counter <= 0;
                ...
                if (p_counter == 312 - 1) begin  // 250 -> 312
                    tick <= 1;
                end
            end else if (p_counter <= 1250 - 1) begin  // 1000->1250
                tick  <= 0;
                o_clk <= 1'b1;
                if (a_reg) begin
                    p_counter <= 0;
                ...
                if (p_counter == 1250 - 1) begin  // 1000->1250
                    p_counter <= 0;
                end
            end
        end
    ...
    always_ff @(posedge tick or posedge reset) begin
        if (reset) begin
            ip_addr_reg  <= 8'b0;
            reg_addr_reg <= 8'b0;
            data_reg     <= 8'b0;
...
module OV7670_set #(
    ...
    always @(posedge clk) begin
        case (addr)
            ...
            74: dout <= 16'h69_07;
            75: begin
                if (mode == "VFLIP") dout <= 16'h1e_10;  // VFLIP = 1
                else dout <= 16'h1e_00;  // VFLIP = 0
            end
